...
clc
%------------------------------------------
% Script to plot DGp1 fourier foots
%------------------------------------------
true_tol = 1.0;
...
% for j=1:length(CFL)
%     
%      [~,mDGfd]= mDG_FourStab(P,Prk, K, Beta, Alpha, CFL(j),true_tol);
%      plot(K, mDGfd.wd1(1,:),'color',ccolor_map(j,:),'linewidth',1.5),hold on
%      
%     % Saving the output:
...
%      
% end
[~,mDGfd]= mDG_FourStab(P,Prk, K, Beta, Alpha, CFL_max.*0.95,true_tol);
[~,DGfd]= DG_FourStab(P,Prk, K, 1.00, 0.333333333333.*0.95,true_tol);

plot(K,mDGfd.wd1(1,:).*CFL_max.*0.95,'-r',K,DGfd.wd1(1,:).*0.95./3.0,'-b','linewidth',1.5)
...
%     
%      [~,mDGfd]= mDG_FourStab(P,Prk, K, Beta, Alpha, CFL(j),true_tol);
%      plot(K, mDGfd.wp1(1,:),'color',ccolor_map(j,:),'linewidth',1.5),hold on
%      
%      % Saving the output:
% %     fname = strcat(dir,'DGp',num2str(P),'_RK',num2str(Prk)....
...
plot(K,mDGfd.wp1(1,:),'-r',K,DGfd.wp1(1,:),'-b','linewidth',1.5),hold on
...
legend({'mDGp1, \beta=0.7','DGp1, \beta=1.00','Exact dispersion'}...
    ,'FontSize',15)
