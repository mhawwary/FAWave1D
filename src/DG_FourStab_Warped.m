...
function [DGsd,DGfd]....
    = DG_FourStab_Warped(Porder,RK_order, Kwavenumber...
    , upwind_param, CFL, true_tol,X)
...
for j=1:Nb   % Loop over Beta 
    for k=1:Nk   % Loop over wavenumber K
        ...
        %------------------------------
        [Asd] = SemiDiscMatrix_1DWaveEqn_warped_elem(P,K(k),Beta(j),X); 
...
%     end
%     
    %[DGfd] = identify_physical_mode_DG(DGfd,P,K,j,true_tol);
